...
const getSubjectLine = (type: string): string => {
  switch (type) {
    ...
    case "quote_request":
      return "Your HUMINEX Custom Quote is Ready";
    case "calculator":
      return "Your HUMINEX Pricing Estimate";
    case "followup":
      return "Following Up on Your HUMINEX Interest";
    default:
      return "Thank You for Your Interest in HUMINEX";
  }
};
...
const getEmailContent = (type: string, name: string, quoteDetails?: QuoteFollowupRequest["quoteDetails"]): string => {
  ...
  switch (type) {
    case "exit_intent":
      contentBlock = `
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          We noticed you were checking out HUMINEX pricing. Before you go, we'd love to offer you an <strong>exclusive 15% discount</strong> on your first 3 months!
        </p>
        <div style="background: linear-gradient(135deg, #00A6A6 0%, #4FF2F2 100%); padding: 20px; border-radius: 12px; margin: 24px 0; text-align: center;">
          <p style="color: white; font-size: 24px; font-weight: bold; margin: 0;">HUMINEX15</p>
          <p style="color: rgba(255,255,255,0.9); font-size: 14px; margin: 8px 0 0 0;">Use this code at checkout</p>
        </div>
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          This offer expires in <strong>48 hours</strong>. Don't miss your chance to experience the power of HUMINEX at a special price.
        </p>
      `;
      break;
    ...
    default:
      contentBlock = `
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          Thank you for your interest in HUMINEX! We're excited to help you transform your workforce management.
        </p>
        <p style="font-size: 16px; color: #374151; line-height: 1.6;">
          Our team will be in touch shortly to discuss how HUMINEX can help your organization.
        </p>
      `;
  }
  ...
  <div style="padding: 20px;">
    <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      ...
      <div style="background: linear-gradient(135deg, #00363D 0%, #0E3A40 100%); padding: 40px 30px; text-align: center;">
        <h1 style="color: white; margin: 0; font-size: 28px;">CropXon HUMINEX</h1>
      ...
      <div style="padding: 40px 30px;">
        ...
        <div style="text-align: center; margin: 32px 0;">
          <a href="https://huminex.cropxon.com/onboarding" 
      ...
      <div style="background: #f8fafc; padding: 24px 30px; text-align: center;">
        ...
        <p style="color: #9ca3af; font-size: 12px; margin: 8px 0 0 0;">
          <a href="https://huminex.cropxon.com/unsubscribe" style="color: #9ca3af;">Unsubscribe</a> | 
          <a href="https://huminex.cropxon.com/privacy" style="color: #9ca3af;">Privacy Policy</a>
        </p>
      </div>
    </div>
  </div>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "HUMINEX <onboarding@resend.dev>",
        to: [email],
